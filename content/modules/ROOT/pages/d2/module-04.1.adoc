...
* Log in using your credentials  - `{user}` and `{password}`.
...
----
globex-db.globex-{user}.svc.cluster.local
----
+
...
// [IMPORTANT]
// ====
// If the output of the `oc whoami` command does not correspond to your username ({user}), you need to log out and log in again with the correct username.

// [source,bash,role=execute,subs="attributes"]
// ----
// oc logout
// oc login -u {user} -p {password} {openshift_api_internal}
// ----
// ====
...
[source,bash,role=execute,subs="attributes"]
----
oc apply -f module-cdc/debezium-connector.yaml -n globex-cdc-{user}
----
+
.Output
----
