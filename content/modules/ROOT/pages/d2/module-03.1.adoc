...
* If you go back to the list of topics in the {streams} *Kafka Console* (click *Topics* on left-hand side) , you will notice a number of topics starting with *globex.recommendation* Those are the topics created by the _recommendation engine_ application, which calculates a list of the most popular products based on the user activity stream using the Kafka Streams API.
+
...
* Click to open the simulator in a new browser tab - https://activity-tracking-simulator-globex-{user}.{openshift_cluster_subdomain}/q/swagger-ui/[Activity Tracking Simulator, window="_simulator"].
...
* In your browser window, navigate to the tab with the *Kafka Console* and notice how messages are produced in the *globex.tracking* topic.
