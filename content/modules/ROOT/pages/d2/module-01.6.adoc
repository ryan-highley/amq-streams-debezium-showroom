...
* In the *lower terminal*, make sure you are on the `kafka-{user}` project.
+
[source,bash,role="execute",subs=attributes+]
----
...
* Navigate to the *Kafka Console > consumer groups > demo-cg* to view the first consumers and partitions being assigned to each consumer.
+
...
* Go back to the *Kafka Console* and observe the partitions being assigned to cg-c1 and cg-c2.  You will see a rebalance happening and partitions getting reassigned between cg-c1 and cg-c2 as shown below.
...
* Go back to the *Kafka Console* and observe the partitions being assigned to cg-c1, cg-c2 and cg-c3.
+
...
* Navigate to the *Kafka Console > topics > consumer groups > demo-cg* and expand consumer cg-c1 we started previously.
