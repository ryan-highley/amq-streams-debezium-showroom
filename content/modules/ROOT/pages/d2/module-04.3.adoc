...
----
btds-kafkacluster-kafka-bootstrap.{user}-tenant-comm-kafka.svc:9092
----
...
[source,bash,role=execute,subs="attributes"]
----
oc apply -f module-cdc/loyalty-deployment.yaml -n globex-{user}
----
...
[source,bash,role=execute,subs="attributes"]
----
oc get pods -n globex-{user} -l app=cdc-dashboard -w
----
...
* Open the link:https://cdc-dashboard-globex-{user}.{openshift_cluster_subdomain}[Loyalty Program Dashboard, window="loyalty"] to access the loyalty program dashboard deployed in the globex-{user}
...
* Feel free to generate more orders in the Globex retail application using the link:https://order-simulator-globex-{user}.{openshift_cluster_subdomain}[Order Simulator, window="order-simulator"] as shown in the previous section, and see how the loyalty points are updated in real-time in the dashboard.
