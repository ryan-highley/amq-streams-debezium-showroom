...
oc run -n {user}-tenant-comm-kafka kafka-toolbox \
  ...
  --restart=Never \
  --env="BOOTSTRAP={kafka_cluster_bootstrap_domain}:9092" \
...
// [source,sh,role="execute",subs=attributes+]
// ----
// oc run -n kafka-{user} topic-describe --rm -it --restart=Never \
...
* Click *Kafka Console* to navigate to the {streams} console to view the topic you created. The console is a web-based user interface for visualizing and monitoring Kafka clusters, topics, messages etc.
